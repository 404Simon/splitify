<x-app-layout>
    <x-form-container title="Edit Recurring Debt: {{ $recurringDebt->name }}">
        <form action="{{ route('groups.recurring-debts.update', [$group, $recurringDebt]) }}" method="POST"
            ...
            <div
                ...
                <a href="{{ route('groups.recurring-debts.show', [$group, $recurringDebt]) }}"
                    class="inline-flex items-center justify-center px-4 py-3 sm:py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-base sm:text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors min-h-[44px] order-2 sm:order-1">
                    Cancel
                </a>
                <x-button class="order-1 sm:order-2 min-h-[44px] text-base sm:text-sm py-3 sm:py-2 justify-center">
        ...
        <div
            class="mt-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-base sm:text-sm font-medium text-yellow-800 dark:text-yellow-200">Important Note
                    </h3>
                    <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                        <p>Changes to the frequency or amount will only affect future generated debts. Previously
                            generated shared debts will remain unchanged.</p>
                    </div>
                </div>
            </div>
        </div>
