<x-app-layout>
                                        ...
                                        <form id="deleteTransactionForm-{{ $transaction->id }}"
                                            ...
                                            <div x-data>
                                                <x-enhanced-button type="button" variant="danger" size="sm"
                                                    x-on:click="$dispatch('open-modal', 'delete-transaction-modal-{{ $transaction->id }}')">
                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
    ...
    @if (!$transactions->isEmpty())
        @foreach ($transactions as $transaction)
            @if ($transaction->payer_id === Auth::id())
                <x-confirmation-modal name="delete-transaction-modal-{{ $transaction->id }}" 
                    title="Delete {{ $transaction->description ?? 'Transaction' }}"
